...
def scene_Taxi_analysis(
    ...
    num_drone = 100,
    simulation_period = (6*60,18*60,1),
    num_simulation = 1,
    is_same_scene = True,
    ...
    if non_default_scene_module is not None:
        ...
        scene = scene_module.Goods_delivery_generator(min_per_step=simulation_period[2],per_RX_base_generate= per_RX_base_generate)
...
def Taxi_simulate_core(
                                num_drone,
                                simulation_period,
                                scene:scene_module.Goods_delivery_generator ,
...
if __name__ == '__main__':
    ...
    order_list ,chargingpower_list ,delta_battery_energy_list  = scene_Taxi_analysis(
        ...
        per_RX_base_generate=per_RX_base_generate,
        power_limit = 700,              #KW
        )
    ...
    # 设置标题
    ax.set_title('charging power of good_delivery drone with ' + charge_strat + ' strategy')
    # 设置备注,保留到整数
