...
class Normal_Drone_Model:
    ...
    def __init__(
                    ...
                    max_battery_energy = None, #最大电池容量
        ...
        if max_battery_energy is None:
            self.max_battery_energy = self.weight /Ct*((self.max_flight_time - self.min_litoff_land_time*2) + TWR_max*self.min_litoff_land_time*2)/60
            if battery_weight is not None and battery_energy_density is not None:
                self.max_battery_energy = battery_weight * battery_energy_density
        else:
            self.max_battery_energy = max_battery_energy
    ...
    def charge_policy(self,period):
        if self.Power_limit is None:
            ...
            self.current_charge_power = self.max_battery_energy/self.max_charge_time*60
            return self.drone_status 
        
        else:
            self.now_battery += self.Power_limit*period/self.max_battery_energy/60
            if self.now_battery >= 1:
                self.now_battery = 1
                self.drone_status = self.status_standby
            self.current_charge_power = self.Power_limit
            return self.drone_status 
    ...
    def output_info(self):
        ...
        print('最大电池容量:',self.max_battery_energy,'KW*h')
        ...
        if self.drone_status == self.status_flight:
            ...
            print('当前充电功率:',self.current_charge_power,'KW')
